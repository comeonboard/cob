...
select *, (select count(*) from project.friends f where m.memIdx = f.memIdxFollowing and memIdxFollow = 12) as followChk from project.member1 m where memIdx = any (select memIdxFollow from project.friends where memIdxFollowing = 12);
